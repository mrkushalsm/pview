...
class DirectoryRenderer:
    ...
    def render(self, path: Path, node_type: NodeType) -> Panel:
        model = ProcTreeModel()
        children = model._sync_get_children(path)
        ...
        # Build preview of first file-like child using renderers
        preview = Text("[dim]No preview available[/dim]")
        try:
            reader = ProcReader()
            registry = RendererRegistry()
            # find first file-ish child (file or symlink)
            preview_candidate = next((c for c in children if c.node_type in (NodeType.FILE, NodeType.SYMLINK)), None)
            if preview_candidate is not None:
                res = reader.read_link(preview_candidate.path) if preview_candidate.node_type == NodeType.SYMLINK else reader.read_text(preview_candidate.path)
                preview = registry.render(preview_candidate.path, res.content)
        except Exception:
            preview = Text("[dim]Preview unavailable[/dim]")

        node_type_label = "Process" if node_type == NodeType.PROCESS else "Directory"
        contents = Group(Text(f"{path}", style="bold cyan"), Text(f"{node_type_label} with {len(children)} entries"), table)

        # Combine listing and preview side-by-side
